...
class Regions(object):
    ...
    def __init__(self, ax, args, input_data_sets, xy_limits, io_manager):
        ...
        print ("regions:")
        #cmto
        if self._args.do_spline == True:
            self._spline = spline_fit.Spline_Fit(
                ax=self._ax,
                args=self._args,
                input_data_sets=self._data_sets,
                xy_limits=xy_limits,
                io_manager=self._io_manager)
            self._spline.plot_spline_curve(self._get_data_sets())
        else:
            self.calculate_fits()
#           print ("regions:\n%s" % pprint.pformat(self.regions))
    ...
    def write_output_files(self):
        ...
        if not self._is_eos_data:
            #cmto
            if self._args.do_spline:
                fit_points = self._spline.get_fit_curve_points()
            else:
            ...
            #cmto
            if self._args.do_spline:
                self._spline.write_derivative_files()
